...
class placetopay(http.Controller):
    ...
    def get_this_acquirer(self):        
        query = "select name, website_id,company_id, state, place2pay_login, secretkey from payment_acquirer where provider = 'place2pay' limit 1"
        request.cr.execute(query)
    ...
    def get_acquirer(self):
        ...
        query = "select name, website_id,company_id, state, place2pay_login from payment_acquirer where provider = 'place2pay' limit 1"
        request.cr.execute(query)
        acquirer = request.cr.dictfetchone()
    ...
    def payment_request(self, **kw):
        ...
        # GET ACQUIRER 
        query = "select id, name, website_id,company_id, state, place2pay_login, secretkey from payment_acquirer where provider = 'place2pay' limit 1"
        request.cr.execute(query)
        acquirer = request.cr.dictfetchone()
