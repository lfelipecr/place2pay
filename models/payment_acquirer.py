...
class payment_acquirer_placetopay(models.Model):
    ...
    provider = fields.Selection(selection_add=[('place2pay', 'PlaceToPay')], default="place2pay", ondelete={'place2pay': 'set default'})
    place2pay_login = fields.Char(string='Login', required_if_provider='place2pay')
    secretkey = fields.Char(string='Secret Key', required_if_provider='place2pay')
    ...
    def render(self, reference, amount, currency_id, partner_id=False, values=None):
        ...
        if(_payment_transaction):
            if(_payment_transaction.acquirer_id.provider == "place2pay"):
